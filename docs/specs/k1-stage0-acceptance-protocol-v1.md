...
- Each cell must have pass rate at least 0.90 and Wilson 95% lower bound at least 0.80. Failures remain in the requested denominator. The supported minimum `n` is the smallest value passing every declared `p` cell for the generic positive and both negative controls.
...
- Seed derivation is `sha256-merge-commit-cell-v1`. Canonical input is `protocol_id|protocol_digest|merge_commit|canonical_cell|replicate_index`; map the first 13 hexadecimal digits to `1 + value mod 2147483645`. Any collision is a protocol failure. Canonical varying-field order is `(n,p)` for generic and both negative controls and `(subjects_per_condition,p)` for AML. Cells are `control=<name>` followed by those base-10 integer `field=value` pairs joined with semicolons. Fixed parameters are excluded because the protocol digest already binds them.
...
- Assert deterministic digest identity, immutable thresholds, per-cell replicate plans, hidden delayed seed derivation, embedded provenance, and rejection of mutation or forged digests.
