...
| [`architecture/scientific-artifact-publication.md`](architecture/scientific-artifact-publication.md) | Immutable filesystem publication, exact manifests, content addressing, cleanup, and generic verification | Payload meaning, scientific validation, claim status, input hashes, RNG identity, or runtime telemetry |
...
| [`agents/visual-palette.md`](agents/visual-palette.md) | Contributor-facing semantic palette roles, permitted data-role exceptions, and plotting-module contract | Scientific estimands or stored evidence |
...
| [`agents/coh-hprcc-deployment.md`](agents/coh-hprcc-deployment.md) | Current cluster-neutral Apollo and Gemini execution through rbiocverse and hprcc, including #193 launch and monitoring | Scientific thresholds, credentials, or cluster-specific paths and partitions |
| [`agents/gemini-hprcc-deployment.md`](agents/gemini-hprcc-deployment.md) | Historical Gemini execution profiles, resource evidence, and completed governed-run record | Current launch instructions or portable cluster policy |
