...
compact_plot <- function(plot) {
    title <- plot$labels$title
    subtitle <- plot$labels$subtitle
    if (is.character(title) && length(title) == 1L &&
        nchar(title) > 32L) {
        title <- paste(strwrap(title, width = 28L), collapse = "\n")
    }
    if (is.character(subtitle) && length(subtitle) == 1L &&
        nchar(subtitle) > 38L) {
        subtitle <- paste(strwrap(subtitle, width = 38L), collapse = "\n")
    }
    plot + ggplot2::theme(
        ...
        ggplot2::guides(
            linewidth = ggplot2::guide_legend(title = "stem width"),
            linetype = ggplot2::guide_legend(title = "stem type")
        )
}
...
for (plot_name in names(plots)) {
    ...
    save_landscapeR_plot(
        compact_plot(plots[[plot_name]]), reduced,
        width_mm = 100,
        height_mm = if (grepl("decomposition", plot_name)) 80 else 64
    )
