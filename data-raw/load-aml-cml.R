...
extract_library_id <- function(sample_names) {
  terminal_id <- sub("^.*_", "", sample_names)
  if (any(!grepl("^[0-9]+$", terminal_id))) {
    stop("AML expression names must end in the numeric library identifier")
  }
  paste0("COHP_", terminal_id)
}

read_aml_sample_weeks <- function(path = AML_SAMPLE_WEEKS) {
  mapping <- utils::read.csv(path, stringsAsFactors = FALSE, check.names = FALSE)
  required <- c(
    "expression_name", "library_id", "source_cohort",
    "prepared_layer", "mouse_id", "sample_weeks"
  )
  if (!identical(names(mapping), required)) {
    stop("AML sample-weeks mapping has an unexpected schema")
  }
  if (anyNA(mapping) || any(!is.finite(mapping$sample_weeks))) {
    stop("AML sample-weeks mapping must be complete and finite")
  }
  if (anyDuplicated(mapping$expression_name) > 0L ||
      anyDuplicated(mapping$library_id) > 0L) {
    stop("AML sample-weeks mapping contains duplicate observation keys")
  }
  mapping$mouse_id <- as.character(mapping$mouse_id)
  mapping
}

attach_aml_sample_weeks <- function(meta, prepared_layer, source_cohort,
                                    mapping = read_aml_sample_weeks()) {
  ...
  if (anyDuplicated(sample_names) > 0L) {
    stop(sprintf("%s contains duplicate expression sample names", prepared_layer))
  }
  ...
  if (!identical(extract_library_id(sample_names), expected$library_id[idx])) {
    stop(sprintf("%s library_id mapping does not match expression names", prepared_layer))
  }
  if (!identical(as.character(meta$mouse_id), expected$mouse_id[idx])) {
    stop(sprintf("%s mouse identifiers disagree with source metadata", prepared_layer))
  }
...
sample_weeks_mapping <- read_aml_sample_weeks()
primary_meta <- attach_aml_sample_weeks(
...
supp_meta <- attach_aml_sample_weeks(
