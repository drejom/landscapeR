test_that("future worker preflight validates declared environment identity", {
    ...
    previous_revision <- Sys.getenv("LANDSCAPER_REVISION", unset = NA_character_)
    on.exit({
        future::plan(previous_plan)
        if (is.na(previous_revision)) Sys.unsetenv("LANDSCAPER_REVISION")
        else Sys.setenv(LANDSCAPER_REVISION = previous_revision)
    }, add = TRUE)
    Sys.setenv(LANDSCAPER_REVISION = "test-revision-134")
    future::plan(future::sequential)

    result <- preflight_future_workers("test-revision-134", workers = 1L)
    expect_s3_class(result, "landscapeR_worker_preflight")
    expect_true(result$workers$ok)
    expect_identical(result$expected$revision, "test-revision-134")
