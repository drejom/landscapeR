...
Association assessment must honour the `SamplingDesign` declared on `StateTransitionData` (ADR 0006). Cross-sectional data use independent-observation methods. Independent destructive time courses declare observed study time but no subject ID; they use a registered fixed-effects condition-by-time strategy and resample canonical biological samples within condition-by-time cells. Repeated-subject time courses declare both subject ID and observed time; adjusted estimates and uncertainty must account for within-subject repeated measures. An ordered cross-sectional state is not observed time and does not make samples longitudinal. Subject identifiers are design variables, not association targets. If a declared design lacks a compatible association strategy, assessment fails explicitly rather than silently treating observations as independent. The atlas records the model, cohort, time transformation, resampling design, and sampling design used.
