...
`stage1-heterogeneous-v2-*`) produced a decisive negative result: `block_scaled_svd` (and `hogsvd_averaged`)
...
n=20, p=400, rank=2, signal=24, noise=1
  ...
  Block-scaled concat SVD (median):0.297   <- `block_scaled_svd` is worse than per-layer
```
...
The v2 protocol compared `hogsvd_averaged` against `block_scaled_svd` as if one of them would be accepted.
...
### 6. `block_scaled_svd` requires re-evaluation

The diagnostic showed `block_scaled_svd`'s median SRE (0.297) is *above* the per-layer oracle
...
- Retain the per-layer pool as the **reference baseline** ((not `block_scaled_svd`))
- Evaluate whether `block_scaled_svd`'s deficit is due to the block-scaling denominator
  (Frobenius norm grows with p, so larger layers dominate after normalisation)
- Evaluate `hogsvd_averaged` (iterative Procrustes consensus) as a candidate only if it
...
Three estimators: per-layer pool, `block_scaled_svd` (concat), `hogsvd_averaged` (consensus).

Results will be used to:
1. Confirm whether `hogsvd_averaged` beats the per-layer floor at any condition
2. Set the final `margin_sre` and `margin_proj` values in section 5 above
3. Determine whether `block_scaled_svd` is recoverable or should be replaced
...
- `block_scaled_svd`: block-scale each layer, concatenate, single SVD
- `hogsvd_averaged`: iterative Procrustes consensus on per-layer SVD scores

```
Conditions where `hogsvd_averaged` beats per-layer pool:  0 / 48
Median `hogsvd_averaged` gain over per-layer:  +0.143  (positive = C1 is WORSE)
```
...
20   80+400 2     96     1     0.032      0.190      0.309   <- K=2: `hogsvd_averaged` diverges
...
1. **`hogsvd_averaged` is genuinely suboptimal**, not just threshold-miscalibrated. It is worse than a
...
2. **`block_scaled_svd` (block-scaled concat) is also worse than the per-layer pool** at all 48 conditions,
...
4. **Both problems are real and separable:**
   ...
   - Algorithm failure: `hogsvd_averaged` and `block_scaled_svd` are genuinely worse than the per-layer pool at all conditions.
...
| `hogsvd_averaged` | **Dropped** | 0/48 wins over per-layer; catastrophic K=2 failure |
| `block_scaled_svd` | **Dropped** | 0/48 wins over per-layer; block-scaling bias confirmed |
