...
| **metadata-association atlas** | A descriptive, serializable table at `omic layer × component × metadata field × association form` grain. Layer identity is never pooled away; unadjusted and adjusted evidence remain separate. | correlation matrix, pooled component score |
| **component-selection proposal** | A reproducible K=1 point ranking by the predeclared direction-aware target effect. Stability validates the top-ranked proposal or causes abstention; it never reranks or promotes a weaker component. Raw or adjusted p-values never determine rank, and Stage 2 results never enter selection. K≥2 input currently yields a structured no-aggregation-strategy abstention rather than hidden cross-layer pooling. | component picker, significance ranking, stability reranking, pooled layer score |
| **multiplicity family** | All component tests for one fixed `omic layer × metadata field × association form × adjustment set`; Holm adjustment is reported within this family as supporting evidence and never used as the proposal rank. | one correction across unrelated metadata questions, p-value ranking |
| **structured abstention** | A typed, serializable result stating that a requested interpretation is not estimable or eligible, with machine-readable reason and retained descriptive evidence. A required model, adjustment, stability condition, or strategy is never replaced by a weaker fallback. | best-effort result, silent fallback, empty result |
| **unadjusted association** | A target-type-specific rank effect computed without nuisance adjustment: signed rank-biserial for binary targets, direction-oriented Spearman for continuous targets, and both descriptive omnibus separation plus predeclared-order Spearman trend for ordered targets. | generic correlation, adjusted effect |
| **residualized rank correlation** | The canonical adjusted cross-sectional effect: correlation between component and target midrank residuals after each is projected onto the complete declared nuisance basis. It is a linear-projection-adjusted marginal-rank estimand, not partial Spearman, conditional independence, or a causal effect. | partial rank association, adjusted rank-biserial, causal effect, fallback correlation |
| **role exclusivity** | Within one declaration, each field has at most one semantic role: target, nuisance, and orientation-anchor roles are mutually exclusive; sampling subject and time roles are distinct. Intrinsic sampling structure and run-specific analysis intent remain separate concerns, so design time may be an explicit target in a time-focused run but never simultaneously a nuisance. | overlapping metadata roles, implicit role promotion |
| **biological exemplar** | A real experimental dataset used after synthetic contract and recovery work to demonstrate operability, diagnostics, face validity, and usability. Without external latent-axis ground truth it cannot establish component-recovery correctness or tune already-frozen acceptance criteria. | ground truth, confirmatory validation dataset |
| **three-rung evidence ladder** | Capability evidence progresses from generic synthetic controls, to domain-grounded synthetic controls, to biological exemplars. Implementation completion, synthetic scientific validation, and biological operability are separate states. | test on real data first, biological plausibility as validation |
| **structural identifiability** | A property of the realized design: required levels and variation exist, the model matrix has full rank and positive residual degrees of freedom, random effects are identifiable, and fitting is non-singular and convergent. It contains no guessed universal sample-size cutoff. | minimum supported sample size, rule-of-thumb n |
| **supported range** | The frozen sample/design envelope where independent synthetic acceptance demonstrates adequate recovery and false-selection behavior. Numeric limits are derived from sweeps, not inferred from structural identifiability or biological plausibility. | structural minimum, optimizer success |
| **SamplingDesign** | The intrinsic collection declaration carried by `StateTransitionData`. Version 2 represents sampling dependence independently from optional observed time: independent/no-time, independent time course, or repeated subject/time. | analysis target, progression model, study plan |
| **sampling dependence** | Whether biological observations are independent units or repeated within a declared subject. It describes data collection and is orthogonal to the biological progression semantics of a target. | progression axis, disease severity |
| **observed collection time** | A measured collection/development/exposure time. It may occur with independent or repeated observations and may be structural sampling information or an explicitly declared analysis target. | pseudotime, ordered disease state |
| **independent time course** | Independent biological observations collected across declared observed times, such as Chen PS/OS cultures or Pogona embryos. Binary trajectory divergence uses a condition × time association without subject random effects. | longitudinal repeated measures, pseudotime series |
| **longitudinal linear-mixed association** | The supported repeated-subject strategy: a standardized fitted-component score-scale target × observed-time model with subject-specific random intercepts and time slopes. Its interaction is a linear slope contrast over the recorded observed-time range, not a rank interaction or general trajectory claim. It supports one subject level; singular fits or more complex dependence structures abstain. | longitudinal rank-mixed association, row-wise regression, random-intercept fallback, universal longitudinal model |
| **design-preserving resampling** | Biological-unit resampling that preserves the declared sampling structure: whole independent observations, observations within independent target × time cells, or whole repeated-subject trajectories within subject-invariant target levels. Rows from repeated subjects are never sampled independently. | row bootstrap, generic resampling |
| **bootstrap component alignment** | Global assignment of resampled K=1 components to the frozen reference by maximum absolute loading cosine, followed only by sign correction from the loading dot product. Axis rotation is measured through separate subspace metrics, never Procrustes-corrected away. | match by PC index, greedy matching, rotate instability away |
| **ordered state** | A predeclared scientific ordering of independent biological groups, such as non-diabetic → autoantibody-positive → T1D. It is not direct within-subject time. | longitudinal progression, pseudotime |
| **continuous severity measure** | A directly measured quantitative correlate such as C-peptide or disease burden that may define a target direction without being time. | observed time, inferred pseudotime |
| **pseudotime** | A derived ordering inferred from molecular observations. It is neither observed collection time nor independent confirmation evidence and cannot silently be promoted into either role. | time, severity, longitudinal observation |
...
| **AnalysisSpecification** | The sole target-intent object for one run: a versioned declaration that retains one complete target contrast/order/direction and nuisance declaration through `draft`, `proposal`, and human-`confirmed` component-selection states. Confirmation adds `selected_component` and decision provenance; it never replaces target intent. | `binary_target`, string-based target/confounder arguments, manual component, target/component XOR, analysis plan (too broad) |
